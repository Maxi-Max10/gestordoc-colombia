...
sap.ui.define([
    ...
    function _replaceVariables(xml, variables, options = {}) {
        ...
        if (options.compactConsecutiveEmptyParagraphs) {
            ...
            // Texto de cada run por separado (no solo el total del párrafo)
            ...
            // Detectar placeholders reales (que existan en el mapa `variables`)
            const regex = /\[\[[^\[\]]*\]\]/g;
            const spans = [];
            let match;
            while ((match = regex.exec(fullText)) !== null) {
                ...
                if (Object.prototype.hasOwnProperty.call(variables, token)) {
                    spans.push({ start: match.index, end: match.index + token.length, value: variables[token] });
                }
            }
            if (spans.length === 0) continue;

            // Posición de inicio de cada run dentro del texto completo del párrafo
            const runStarts = [];
            let pos = 0;
            for (const t of runTexts) { runStarts.push(pos); pos += t.length; }

            const spanEmitted = spans.map(() => false); // el reemplazo se escribe una sola vez por span

            for (let r = 0; r < runs.length; r++) {
                const run = runs[r];
                const rStart = runStarts[r];
                const rText = runTexts[r];
                const rEnd = rStart + rText.length;

                const overlapping = spans
                    .map((sp, idx) => ({ sp, idx }))
                    .filter(({ sp }) => sp.start < rEnd && sp.end > rStart);

                if (overlapping.length === 0) continue; // este run no toca ningún placeholder: se deja intacto (formato incluido)

                overlapping.sort((a, b) => a.sp.start - b.sp.start);

                let newText = "";
                let cursor = rStart;
                for (const { sp, idx } of overlapping) {
                    const segStart = Math.max(sp.start, rStart);
                    const segEnd = Math.min(sp.end, rEnd);

                    if (cursor < segStart) {
                        newText += rText.slice(cursor - rStart, segStart - rStart);
                    }
                    if (!spanEmitted[idx]) {
                        newText += sp.value;
                        spanEmitted[idx] = true;
                    }
                    cursor = segEnd;
                }
                if (cursor < rEnd) {
                    newText += rText.slice(cursor - rStart, rEnd - rStart);
                }

                // Volcar newText en este run, conservando SU propio formato (rPr propio, no el de otro run)
                const ts = Array.from(run.children).filter(el => el.localName === "t");
                for (let k = 1; k < ts.length; k++) run.removeChild(ts[k]);

                let targetT = ts[0];
                if (!targetT) {
                    targetT = doc.createElementNS(NS, "w:t");
                    run.appendChild(targetT);
                }
