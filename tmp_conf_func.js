async function onDownloadPDFConfidencialidad(oController, sButtonId) {
      try {
        ...
        var aUsers = oController._getSelectedUsers();
        if (aUsers.length === 0) {
          MessageToast.show("Seleccione al menos un colaborador.");
          return;
        }
        for (let user of aUsers) {
          var oElement = document.createElement("div");
          oElement.style.width = "800px";
          oElement.style.padding = "20px";
          const segundoNombre = user.secondLastName != "" ? user.secondLastName : " "
          const marital = user.maritalStatus ? user.maritalStatus : "COMPLETAR"
  ...
  <li>
    ...
    <p style='margin:0cm;font-size:16px;font-family:"Times New Roman",serif;text-align:center;'><span style='font-size:15px;font-family:"Calibri",sans-serif;color:black;'>__________________________________</span></p>
          ...
          if (sButtonId === "container-gestordoc---View1--wordDataInfo") {
            ...
            oController._downloadPDF(pdfBytes, `${user.firstName}_${user.lastName}_Confidencialidad.pdf`);
